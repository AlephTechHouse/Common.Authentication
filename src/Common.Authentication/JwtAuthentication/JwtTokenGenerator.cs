...
public class JwtTokenGenerator
{
    ...
    public Task<AuthenticationResponse?> GenerateJwtToken(JwtTokenRequest jwtTokenRequest)
    {
        ...
        var tokenKey = Encoding.ASCII.GetBytes(JwtSecurityKey.Value);
        var claimsIdentity = new ClaimsIdentity(new List<Claim>
        {
            new Claim(nameof(JwtRegisteredClaimNames.Name), jwtTokenRequest.UserId.ToString()!),
            new Claim(nameof(JwtTokenRequest.RolesIds), jwtTokenRequest.RolesIds!.ToString()!)
        });
        ...
        return Task.FromResult<AuthenticationResponse?>(new AuthenticationResponse
        {
            ...
            RolesIds = jwtTokenRequest.RolesIds,
            JwtToken = jwtToken,
